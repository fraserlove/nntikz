...
\newcommand{\LSTMCell}[2]{
   ...
   % Draw opérators   named mux# , add# and func#
    ...
    \node[] (c) at (-3.75,1.5) {};
    \node[] (h) at (-3.75,-1.5) {};
    \node[circle, draw, minimum width=1cm, thick, align=center, fill=gray!10] (x) at (-2.5,-3) {$\mathbf{x}_{#1}$};
...
% Start connecting all.
    ...
    % Add the semi-transparent overlay and LSTM label
...
\begin{scope}[scale=0.7, transform shape, shift={(6,0)}]
    %\LSTMCell{{T_{\text{in}}}}{y}
    \LSTMCell{3}{y}{n}
\end{scope}

% Add the context vector block
\node[rectangle, draw, thick, minimum height=2.8cm, minimum width=0cm, fill=gray!10] (context) at (7.2,0) {$\mathbf{c}$};
%\node[] (context) at (1.75,0) {\scalebox{1.5}{$\boldmath{\cdots}$}};
\end{tikzpicture}
